"""Silent BANT classifier sub-agent."""
...
class BantSignals(BaseModel):
    """Structured BANT output consumed by the Amaru orchestrator."""
    ...
    plan_and_date_confirmed: bool = Field(
        description=(
            "True only with a published plan, a listed departure of that plan, "
            "and acceptance to book that departure (explicit reserve phrase, "
            "or yes after Amaru asked to reserve that outing)."
        ),
    )
    custom_group_accepted: bool = Field(
        default=False,
        description=(
            "True only if the client accepted assembling a custom group or date "
            "after Amaru offered it. False when they only say no published date fits."
        ),
    )
    disability_access_inquiry: bool = Field(
        default=False,
        description=(
            "True if they ask whether they or a companion can attend with a "
            "disability, or request trip accessibility. False if they only say "
            "the plan is tiring."
        ),
    )
...
qualifier_agent = Agent(
    ...
    description="Silent BANT signal extractor for Amaru.",
