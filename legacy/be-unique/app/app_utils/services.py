...
# ADK treats GOOGLE_GENAI_USE_VERTEXAI=1 as enterprise/express mode and then
# feeds GOOGLE_API_KEY into vertexai.Client. SessionService.GetSession rejects
# API keys (401 CREDENTIALS_MISSING). Agent Runtime identity is ADC.
_SESSION_ADC_STRIP_ENV = ("GOOGLE_API_KEY",)
...
def _vertex_ai_session_service(agent_engine_id: str):
    """Vertex session client on ADC. Does not change GOOGLE_CLOUD_LOCATION."""
    for key in _SESSION_ADC_STRIP_ENV:
    ...
    return VertexAiSessionService(
        ...
        # (which agent.py pins to "global" for Gemini models).
...
def get_session_service():
    ...
    if agent_engine_id := os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID"):
        return _vertex_ai_session_service(agent_engine_id)
