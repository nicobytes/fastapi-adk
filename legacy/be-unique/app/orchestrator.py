"""Be Unique BaseAgent orchestrator: qualifier → Python handoff → bridge | customer."""
...
def _handoff_reason(criteria: dict[str, Any]) -> str:
    if criteria.get("explicit_human_request"):
        return "Cliente solicitó atención humana."
    if criteria.get("plan_and_date_confirmed"):
        return "Servicio y fecha confirmados para cita."
...
class BeUniqueOrchestrator(BaseAgent):
    ...
    customer: Agent = Field(description="Customer-facing Sofía chat sub-agent.")
        ...
        if qualifier_ok:
            ...
            if criteria is not None:
                ...
                handoff = maybe_execute_handoff(
                    ...
                    explicit_human_request=bool(score_result["explicit_human_request"]),
                )
...
def create_be_unique_orchestrator(
    *,
    ...
    | None = None,
) -> BeUniqueOrchestrator:
    """Factory to avoid ADK parent-agent registration issues at import time."""
    qualifier = qualifier_agent
    bridge = bridge_agent
    activate = activate_agent
    customer = customer_agent
    return BeUniqueOrchestrator(
        name="be_unique",
        description=(
            "Be Unique WhatsApp orchestrator: BANT via qualifier, "
            "Python handoff (explicit human only), bridge or Sofía customer chat."
        ),
        qualifier=qualifier,
        bridge=bridge,
        activate=activate,
