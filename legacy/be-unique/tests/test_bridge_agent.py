...
def test_build_instruction_renders_bridge_example_and_reason() -> None:
    with patch(
        ...
        return_value="Le conecto con Laura del equipo.",
    ):
        ctx = _FakeReadonlyContext(
            {"handoff_reason": "Cliente solicitó atención humana."}
        )
        text = asyncio.run(build_instruction(ctx))  # type: ignore[arg-type]

    assert "Cliente solicitó atención humana." in text
    assert "Le conecto con Laura del equipo." in text
